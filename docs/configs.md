...
| `RandomizeCostume` | `Default` | キャストの初期衣装を変更<br>Random： キャストの初期衣装をランダムにする<br>Casual： 1/5の確率で私服にする<br>Default： 通常状態<br>（注意）GBSystem.Instance.m_costumeOverride を上書きするのでデフォルト以外は衣装変更が効かなくなる |

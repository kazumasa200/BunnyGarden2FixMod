...
| キー | デフォルト | 説明 |
|------|-----------|------|
| `Sensitivity` | `2` | フリーカメラのマウス感度 |
| `Speed` | `10` | フリーカメラの移動速度 |
| `FastSpeed` | `30` | 高速移動速度（Shift 押しながら移動） |
| `SlowSpeed` | `2.5` | 低速移動速度（Ctrl 押しながら移動） |
...
> **注意**: 高解像度データはゲーム本体とは別のサイドカーファイル（`セーブファイル名.exmod`）に保存されます。Steam Cloud Save には同期されないため、PCを移行する場合は手動でコピーしてください。MODを外しても本体セーブ（320×320版）は破損しません。
